...
- **Non-determinism:** runtime LLM means outputs can vary run-to-run; the tool stubs are deterministic, but judgment is not. The levers I'd reach for: drop sampling temperature toward 0 for reproducibility, and add a multi-sample agreement check to the eval so run-to-run stability (especially on urgency) becomes a gated metric rather than something eyeballed once. (I left temperature at the SDK default here — on the borderline cases like a blank referral, `new_referral` vs `missing_paperwork` is a genuine judgment call, so I'd want the eval to *measure* that variance before pinning it, not silently freeze one sample.)
